...
export default function Documents() {
  ...
  const filteredDocuments = documents.filter((doc: Document) => {
    ...
    const matchesPersonnel = !selectedPersonnel || doc.personnelId.toString() === selectedPersonnel;
    const matchesType = !documentTypeFilter || doc.documentType === documentTypeFilter;
  ...
  const expiringSoonDocs = documents.filter((doc: Document) => {
    const days = getDaysUntilExpiration(doc.expirationDate);
    return days >= 0 && days <= 30;
  }).length;

  const validDocs = documents.filter((doc: Document) => 
